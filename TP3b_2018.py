...
class Gear(object):
    """
    This class defines a gear object with its physical properties. The Harmonograph 
    is built out of two gear-objects.
    """

    def __init__(self,x,y,r,pradial,pangular,speed,r2,armLenght,armAngle):  
        self.x=x
        self.y=y
        self.r=r
        ...
        self.speed=speed
        #position of the pivot point of the arm is found through trigonometry
    ...
    def rotate(self):
        """
        This function makes the gear rotate, together with the pivot point of the 
        drawing arm
        """
    ...
    def armEnd(self, anchorX, anchorY):
        """
        This function recovers the end point of the drawing arm thorugh trigonometry
        """
    ...
    def areLegalGears(self, other):
        """
        This function checks if the position of two drawing arms (from two gears) is 
        physically possible. If you can form a triangle, then it's legal.
        Returns True if arms are legal. Returns None if not.
        """
    ...
    def draw(self, canvas):
        """
        This function creates the inetrface for the rotating gears on Tkinter canvas
        """
...
class Button(object):
    """
    This function defines the button objects for the user to interact with
    """
...
class Paper(object):
    """
    This function defines the paper surface of which the Harmonograph will be drawn. 
    It is also the domain for the end point of the drawing arms.
    """
...
settings=33   #user setting can be one of [1, 2, 3, 4, 11, 22, 33, 44, 111, 222,
                # 333, 444] these correspond to different drawing modes
...
def init(data):
    ...
    if data.userSettings==1:
        ...
        data.gear1pradial=data.gear1radius-30
        data.gear2pradial=data.gear2radius-30
        data.gear1pangular=math.pi*4/2*2
        data.gear2pangular=-math.pi*1/2*2
        data.gear1speed=data.gear1pangular
        data.gear2speed=data.gear2pangular
        data.gearJointRadius=5
...
def loadHarmonograms(data):
    """
    This function loads the harmonograms
    """     
    Harmonograms=2
...
def loadGears(data):
    """
    This function loads the decoration images of the gears, which change in 
    colors based on color setting
    """     
...
def findIntersectionOfCircles(data):
    """
    returns only the arm intersection of interest. This is the intersection 
    of two circles, which can be a set of 0, 1 or 2 points. In the case of 2
    intersection, one is disregarded as it is outside the drawing bound
    """
    x1=data.anchor1x
    y1=data.anchor1y
    x2=data.anchor2x
...
def rotatePoint(data, x, y):
    """
    updates the position of the interest point in input by rotating it of a small
    angle
    """
...
def getPoint(data):
    """
    mathematical formula for Zen mode, hardcoded physical behavior.
    Used in zenTimerFired
    """
    cX=data.width/2
    cY=data.height/2
...
def harmonographTimerFired(data):
    ...
    if data.stopDrawing==False:
        ...
        data.anchor2x=data.gear2.armAnchor()[0]
        data.anchor2y=data.gear2.armAnchor()[1]
        #identify two circles whose intersection is the drawing point
...
def drawInfoText(canvas, data):
    text="A harmonograph is a drawing machine \n controlled by the motion of pendulums or rotating disks. \nIt generates beautiful and complex curves - ellipses, spirals, \n figure eights, Lissajous figures."
    ellipses="ellipses"
    spirals="spirals"
